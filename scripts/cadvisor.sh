...
# app variables
APP_VERSION="$(curl -s https://api.github.com/repos/google/cadvisor/releases/latest | jq -r .tag_name)"
...
if [ "${APP_UPGRADE}" = true ]; then
    ...
    # copy binaries
    install -m 0755 -o root -g root ${TEMP_DIR}/cadvisor-${APP_VERSION}-linux-${SYSTEM_ARCH} /usr/local/bin/cadvisor
