...
APP_VERSION="$(curl -s https://api.github.com/repos/prometheus/blackbox_exporter/releases/latest | jq -r .tag_name)"
...
if [ "${APP_UPGRADE}" = true ]; then
    ...
    # copy binaries
    cp ${TEMP_DIR}/blackbox_exporter-${APP_VERSION#v}.linux-${SYSTEM_ARCH}/blackbox_exporter /usr/local/bin/blackbox_exporter

    # update ownership
    chown ${APP_SA}:${APP_SA} /usr/local/bin/blackbox_exporter
