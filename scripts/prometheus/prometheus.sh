...
PROMETHEUS_VERSION_SHORT="${PROMETHEUS_VERSION#v}"

# prometheus upgrade variables
PROMETHEUS_UPGRADE=false
...
# status message
echo -e "\033[0;92mINFO:\033[0m downloading prometheus ${PROMETHEUS_VERSION} for ${SYSTEM_ARCH} architecture"

# download binaries
curl -sLo ${TEMP_DIR}/prometheus-${PROMETHEUS_VERSION_SHORT}.linux-${SYSTEM_ARCH}.tar.gz https://github.com/prometheus/prometheus/releases/download/${PROMETHEUS_VERSION}/prometheus-${PROMETHEUS_VERSION_SHORT}.linux-${SYSTEM_ARCH}.tar.gz
tar -xzf ${TEMP_DIR}/prometheus-${PROMETHEUS_VERSION_SHORT}.linux-${SYSTEM_ARCH}.tar.gz --directory ${TEMP_DIR}

### upgrade

if [ "${PROMETHEUS_UPGRADE}" = true ]; then
    # status message
    ...
    # copy binaries
    cp ${TEMP_DIR}/prometheus-${PROMETHEUS_VERSION_SHORT}.linux-${SYSTEM_ARCH}/prometheus /usr/local/bin/prometheus
    cp ${TEMP_DIR}/prometheus-${PROMETHEUS_VERSION_SHORT}.linux-${SYSTEM_ARCH}/promtool /usr/local/bin/promtool

    # update ownership
    chown ${PROMETHEUS_SA}:${PROMETHEUS_SA} /usr/local/bin/prometheus
    chown ${PROMETHEUS_SA}:${PROMETHEUS_SA} /usr/local/bin/promtool
...
useradd --system --no-create-home --shell /bin/false ${PROMETHEUS_SA}
...
cat << 'EOF' > /etc/prometheus/rules/pushgateway.yaml
groups:
- name: pushgateway
  rules:

  - alert: pushgateway_too_many_restarts
    expr: 'changes(process_start_time_seconds{job="pushgateway"}[15m]) > 2'
    for: 0m
    labels:
      severity: warning
    annotations:
      summary: "Pushgateway too many restarts"
      description: "Pushgateway has restarted more than twice in the last 15 minutes."
EOF
...
chown ${PROMETHEUS_SA}:${PROMETHEUS_SA} /usr/local/bin/prometheus
chown ${PROMETHEUS_SA}:${PROMETHEUS_SA} /usr/local/bin/promtool
chown -R ${PROMETHEUS_SA}:${PROMETHEUS_SA} /etc/prometheus
chown -R ${PROMETHEUS_SA}:${PROMETHEUS_SA} /var/lib/prometheus
