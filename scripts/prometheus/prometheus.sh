...
# app variables
APP_SA="prometheus"
APP_VERSION="$(curl -s https://api.github.com/repos/prometheus/prometheus/releases/latest | jq -r .tag_name)"
...
if [ "${APP_UPGRADE}" = true ]; then
    ...
    # copy binaries
    cp ${TEMP_DIR}/prometheus-${APP_VERSION#v}.linux-${SYSTEM_ARCH}/prometheus /usr/local/bin/prometheus
    cp ${TEMP_DIR}/prometheus-${APP_VERSION#v}.linux-${SYSTEM_ARCH}/promtool /usr/local/bin/promtool

    # update ownership
    chown ${APP_SA}:${APP_SA} /usr/local/bin/prometheus
    chown ${APP_SA}:${APP_SA} /usr/local/bin/promtool
