...
APP_VERSION="$(curl -s https://api.github.com/repos/prometheus/node_exporter/releases/latest | jq -r .tag_name)"
...
if [ "${APP_UPGRADE}" = true ]; then
    ...
    # copy binaries
    cp ${TEMP_DIR}/node_exporter-${APP_VERSION#v}.linux-${SYSTEM_ARCH}/node_exporter /usr/local/bin/node_exporter

    # update ownership
    chown ${APP_SA}:${APP_SA} /usr/local/bin/node_exporter
