...
# app variables
APP_VERSION="$(curl -s https://api.github.com/repos/slok/sloth/releases/latest | jq -r .tag_name)"
...
chmod +x ${TEMP_DIR}/sloth-linux-${SYSTEM_ARCH}
...
if [ "${APP_UPGRADE}" = true ]; then
    ...
    # copy binaries
    cp ${TEMP_DIR}/sloth-linux-${SYSTEM_ARCH} /usr/local/bin/sloth
